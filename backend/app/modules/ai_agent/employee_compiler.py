...
- Use await_event only when the Job Brief explicitly says later steps in the same job must wait for a future Xvond/internal/provider event instead of starting a separate unrelated workflow. Put the stable event name in params.event and include params.source_text copied verbatim from the Job Brief words that authorize waiting for that event. Use params.match for correlation when the workflow is waiting for an event belonging to a specific order, lead, payment, booking or other entity; match values may reference $input.* or previous node outputs. Never invent an event wait. Do not use await_event when the event merely starts the job; use execution_graph.trigger type=event for that case.
...
def _ground_execution_graph(value: Any, *, job_brief: str) -> dict:
    """Ground timing/event pauses in exact customer language before execution."""
    ...
    def visit(raw_graph: Any) -> dict:
        ...
        for node in graph.get("nodes") or []:
            ...
            node_type = str(current.get("type") or "").strip().lower()
            params = (
