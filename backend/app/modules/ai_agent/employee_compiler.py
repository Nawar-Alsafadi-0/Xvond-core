...
- Use foreach only when the complete finite items list already exists. Use repeat only when the next iteration depends on the previous iteration result, such as cursor/page-token pagination or another explicitly requested bounded iterative protocol. repeat params must include max_iterations from 1 to 20, a nested graph, and until={path,operator,value}; path is relative to the nested graph result. The nested graph may read the prior iteration result through $previous.* and the zero-based iteration through $index. initial may seed $previous for the first iteration. Never emit an unbounded loop. When exhaustive completion matters, treat $nodes.<repeat_id>.limit_reached=true as incomplete rather than claiming the loop finished normally.
