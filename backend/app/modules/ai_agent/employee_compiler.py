...
Use this shape:
{
  ...
  "execution_graph": {
    ...
    "nodes": [
      {
        "id": "stable_node_id",
        "type": "ai|media|action|http_get_json|web_fetch|browser|transform|condition|notify|wait|await_event|foreach|repeat|select|filter|aggregate|state_read|state_write|state_delete",
        "depends_on": [],
        "params": {}
      }
    ]
...
- Do not split sequential steps of the same job into separate routines. If work is one continuous lifecycle (including wait, await_event, approval, foreach or repeat checkpoints), keep it inside one graph.
...
- Use foreach only when the complete finite items list already exists. Use repeat only when the next iteration depends on the previous iteration result, such as cursor/page-token pagination or another explicitly requested bounded iterative protocol. repeat params must include max_iterations from 1 to 20, a nested graph, and until={path,operator,value}; path is relative to the final node output (graph_last) of each iteration. The next iteration reads that final output through $previous.* and the zero-based iteration through $index. initial may seed $previous for the first iteration. Full per-iteration graph results remain available in the repeat node's iterations output. Never emit an unbounded loop. When exhaustive completion matters, treat $nodes.<repeat_id>.limit_reached=true as incomplete rather than claiming the loop finished normally.
