...
MICROSOFT_TEAMS_WORKFLOW_FILE="${MICROSOFT_TEAMS_WORKFLOW_FILE:-ops/n8n/xvond-microsoft-teams-provider.workflow.json}"
MICROSOFT_TEAMS_WORKFLOW_ID="${MICROSOFT_TEAMS_WORKFLOW_ID:-xvond-microsoft-teams-provider-v1}"

for file in "$ACTION_WORKFLOW_FILE" "$CHANNEL_WORKFLOW_FILE" "$TELEGRAM_WORKFLOW_FILE" "$META_WORKFLOW_FILE" "$SLACK_WORKFLOW_FILE" "$CUSTOM_CHANNEL_WORKFLOW_FILE" "$TWILIO_SMS_WORKFLOW_FILE" "$MAILGUN_EMAIL_WORKFLOW_FILE" "$MICROSOFT_TEAMS_WORKFLOW_FILE"; do
    if [ ! -f "$file" ]; then
...
probe_microsoft_teams_gateway() {
    docker exec xvond-workflow-engine node -e '
fetch("http://127.0.0.1:5678/webhook/xvond-microsoft-teams-provider", {
  method: "POST",
  headers: {"content-type": "application/json"},
  body: JSON.stringify({action: "health_probe"}),
}).then(async response => {
  ...
  if (!result || result.success !== false) {
    console.error(`invalid_microsoft_teams_gateway_response:${text.slice(0, 300)}`);
    process.exit(2);
  }
  process.exit(0);
}).catch(error => {
...
wait_for_runtime_webhooks() {
    ...
    while [ "$count" -lt "$attempts" ]; do
        if action_output="$(probe_action_gateway 2>&1)" &&
           ...
           mailgun_output="$(probe_mailgun_email_gateway 2>&1)" &&
           teams_output="$(probe_microsoft_teams_gateway 2>&1)"; then
            return 0
        else
            code="$?"
...
sync_one_workflow "$MICROSOFT_TEAMS_WORKFLOW_FILE" "$MICROSOFT_TEAMS_WORKFLOW_ID"
