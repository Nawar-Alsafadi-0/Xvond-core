...
- **Slack** — uses a source-controlled Slack Events API/Web API provider workflow on the shared Managed Channel Gateway. Requests are verified from the raw request body with Slack signing-secret HMAC and five-minute replay protection; outbound `chat.postMessage` must return Slack `ts` before Xvond accepts delivery. Slack still requires a real workspace installation, scopes/event subscription and external round-trip acceptance before the exact customer channel is called service-ready.
...
- **Microsoft Teams** — the shared Xvond Managed Channel Gateway runtime exists, but Teams still requires a provider-specific Xvond binding. It must not be presented as a packaged live connector until a source-controlled provider binding and real external acceptance exist.
...
7. After those external gates pass, the reviewed release can be truthfully exposed as the public/global Xvond AI Employee launch. Microsoft Teams remains custom Xvond setup until a packaged provider binding is intentionally added. Email now has a Mailgun packaged binding; SMS has a Twilio packaged binding; Slack and Custom/API also have packaged source-controlled bindings. All still require real external acceptance before sale.
